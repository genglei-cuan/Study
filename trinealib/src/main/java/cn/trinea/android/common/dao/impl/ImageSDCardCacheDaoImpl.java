...
public class ImageSDCardCacheDaoImpl implements ImageSDCardCacheDao {
     ...
     * @return
     */
    private static ContentValues cacheObjectToCV(String tag, String url,
    ...
    public boolean putIntoImageSDCardCache(ImageSDCardCache imageSDCardCache, String tag) {
        ...
        Cursor
            cursor =
            sqliteUtils.getDb().query(DbConstants.IMAGE_SDCARD_CACHE_TABLE_TABLE_NAME, null,
        ...
        if (cursor != null && cursor.getCount() > 0) {
            for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext()) {
                ...
                value.setUsedCount(
                    cursor.getInt(DbConstants.IMAGE_SDCARD_CACHE_TABLE_USED_COUNT_INDEX));
                value.setPriority(
                    cursor.getInt(DbConstants.IMAGE_SDCARD_CACHE_TABLE_PRIORITY_INDEX));
                value.setExpired(
                    cursor.getInt(DbConstants.IMAGE_SDCARD_CACHE_TABLE_IS_EXPIRED_INDEX) == 1);
                value.setForever(
                    cursor.getInt(DbConstants.IMAGE_SDCARD_CACHE_TABLE_IS_FOREVER_INDEX) == 1);
    ...
    public boolean deleteAndInsertImageSDCardCache(ImageSDCardCache imageSDCardCache, String tag) {
        ...
        try {
            ...
            db.delete(DbConstants.IMAGE_SDCARD_CACHE_TABLE_TABLE_NAME, whereClause.toString(),
                      whereArgs);
            ...
            for (Entry<String, CacheObject<String>> entry : imageSDCardCache.entrySet()) {
                if (entry != null && (key = entry.getKey()) != null
                    ...
                    db.insert(DbConstants.IMAGE_SDCARD_CACHE_TABLE_TABLE_NAME, null,
                              cacheObjectToCV(tag, key, value));
                }
            }
