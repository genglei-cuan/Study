...
/**
 * DownloadManagerPro <ul> <strong>Get download info</strong> <li>{@link #getStatusById(long)} get
 * download status</li> <li>{@link #getDownloadBytes(long)} get downloaded byte, total byte</li>
 * <li>{@link #getBytesAndStatus(long)} get downloaded byte, total byte and download status</li>
 * <li>{@link #getFileName(long)} get download file name</li> <li>{@link #getUri(long)} get download
 * uri</li> <li>{@link #getReason(long)} get failed code or paused reason</li> <li>{@link
 * #getPausedReason(long)} get paused reason</li> <li>{@link #getErrorCode(long)} get failed error
 * code</li> </ul> <ul> <strong>Operate download</strong> <li>{@link #isExistPauseAndResumeMethod()}
 * whether exist pauseDownload and resumeDownload method in {@link DownloadManager}</li> <li>{@link
 * #pauseDownload(long...)} pause download. need pauseDownload(long...) method in {@link
 * DownloadManager}</li> <li>{@link #resumeDownload(long...)} resume download. need
 * resumeDownload(long...) method in {@link DownloadManager}</li> </ul> <ul>
 * <strong>RequestPro</strong> <li>{@link RequestPro#setNotiClass(String)} set noti class</li>
 * <li>{@link RequestPro#setNotiExtras(String)} set noti extras</li> </ul>
 *
...
public class DownloadManagerPro {
    ...
    private static void initPauseMethod() {
        ...
        try {
            pauseDownload =
                DownloadManager.class.getMethod(METHOD_NAME_PAUSE_DOWNLOAD, long[].class);
        } catch (Exception e) {
    ...
    private static void initResumeMethod() {
        ...
        try {
            resumeDownload =
                DownloadManager.class.getMethod(METHOD_NAME_RESUME_DOWNLOAD, long[].class);
        } catch (Exception e) {
            // accept all exception
            e.printStackTrace();
        }
     ...
     *
     * @return a int array with two elements <ul> <li>result[0] represents downloaded bytes, This
     * will initially be -1.</li> <li>result[1] represents total bytes, This will initially be
     * -1.</li> </ul>
     */
     ...
     *
     * @return a int array with three elements <ul> <li>result[0] represents downloaded bytes, This
     * will initially be -1.</li> <li>result[1] represents total bytes, This will initially be
     * -1.</li> <li>result[2] represents download status, This will initially be 0.</li> </ul>
     */
     ...
     *
     * @return <ul> <li>if status of downloadId is {@link DownloadManager#STATUS_PAUSED}, return
     * {@link #getPausedReason(long)}</li> <li>if status of downloadId is {@link
     * DownloadManager#STATUS_FAILED}, return {@link #getErrorCode(long)}</li> <li>if status of
     * downloadId is neither {@link DownloadManager#STATUS_PAUSED} nor {@link
     * DownloadManager#STATUS_FAILED}, return 0</li> </ul>
     */
     ...
     * get paused reason
     *
     * @return <ul> <li>if status of downloadId is {@link DownloadManager#STATUS_PAUSED}, return one
     * of {@link DownloadManager#PAUSED_WAITING_TO_RETRY}<br/> {@link DownloadManager#PAUSED_WAITING_FOR_NETWORK}<br/>
     * {@link DownloadManager#PAUSED_QUEUED_FOR_WIFI}<br/> {@link DownloadManager#PAUSED_UNKNOWN}</li>
     * <li>else return {@link DownloadManager#PAUSED_UNKNOWN}</li> </ul>
     */
