...
/**
 * PackageUtils <ul> <strong>Install package</strong> <li>{@link PackageUtils#installNormal(Context,
 * String)}</li> <li>{@link PackageUtils#installSilent(Context, String)}</li> <li>{@link
 * PackageUtils#install(Context, String)}</li> </ul> <ul> <strong>Uninstall package</strong>
 * <li>{@link PackageUtils#uninstallNormal(Context, String)}</li> <li>{@link
 * PackageUtils#uninstallSilent(Context, String)}</li> <li>{@link PackageUtils#uninstall(Context,
 * String)}</li> </ul> <ul> <strong>Is system application</strong> <li>{@link
 * PackageUtils#isSystemApplication(Context)}</li> <li>{@link PackageUtils#isSystemApplication(Context,
 * String)}</li> <li>{@link PackageUtils#isSystemApplication(PackageManager, String)}</li> </ul>
 * <ul> <strong>Others</strong> <li>{@link PackageUtils#getInstallLocation()} get system install
 * location</li> <li>{@link PackageUtils#isTopActivity(Context, String)} whether the app whost
 * package's name is packageName is on the top of the stack</li> <li>{@link
 * PackageUtils#startInstalledAppDetails(Context, String)} start InstalledAppDetails Activity</li>
 * </ul>
 *
...
public class PackageUtils {
    ...
    /**
     * Installation return code<br/> the package manager service found that the device didn't have
    ...
    /**
     * Installation return code<br/> a previously installed package of the same name has a different
     * signature than the new package (and the old package's data was not removed).
     */
    public static final int INSTALL_FAILED_UPDATE_INCOMPATIBLE = -7;
    /**
     * Installation return code<br/> the new package is requested a shared user which is already
    ...
    /**
     * Installation return code<br/> the new package failed while optimizing and validating its dex
     * files, either because there was not enough storage or the validation failed.
     */
    public static final int INSTALL_FAILED_DEXOPT = -11;
    /**
     * Installation return code<br/> the new package failed because the current SDK version is older
     * than that required by the package.
     */
    public static final int INSTALL_FAILED_OLDER_SDK = -12;
    /**
     * Installation return code<br/> the new package failed because it contains a content provider
     * with the same authority as a provider already installed in the system.
     */
    public static final int INSTALL_FAILED_CONFLICTING_PROVIDER = -13;
    /**
     * Installation return code<br/> the new package failed because the current SDK version is newer
     * than that required by the package.
     */
    public static final int INSTALL_FAILED_NEWER_SDK = -14;
    /**
     * Installation return code<br/> the new package failed because it has specified that it is a
     * test-only package and the caller has not supplied the {@link #INSTALL_ALLOW_TEST} flag.
     */
    public static final int INSTALL_FAILED_TEST_ONLY = -15;
    /**
     * Installation return code<br/> the package being installed contains native code, but none that
     * is compatible with the the device's CPU_ABI.
     */
    ...
    /**
     * Installation return code<br/> a secure container mount point couldn't be accessed on external
     * media.
     */
    public static final int INSTALL_FAILED_CONTAINER_ERROR = -18;
    /**
     * Installation return code<br/> the new package couldn't be installed in the specified install
     * location.
     */
    ...
    /**
     * Installation return code<br/> the new package couldn't be installed because the verification
     * timed out.
     */
    ...
    /**
     * Installation return code<br/> the new package is assigned a different UID than it previously
     * held.
     */
    public static final int INSTALL_FAILED_UID_CHANGED = -24;
    /**
     * Installation return code<br/> if the parser was given a path that is not a file, or does not
     * end with the expected '.apk' extension.
     */
    public static final int INSTALL_PARSE_FAILED_NOT_APK = -100;
    /**
     * Installation return code<br/> if the parser was unable to retrieve the AndroidManifest.xml
     * file.
     */
    ...
    /**
     * Installation return code<br/> if the parser found inconsistent certificates on the files in
     * the .apk.
     */
    public static final int INSTALL_PARSE_FAILED_INCONSISTENT_CERTIFICATES = -104;
    /**
     * Installation return code<br/> if the parser encountered a CertificateEncodingException in one
     * of the files in the .apk.
     */
    public static final int INSTALL_PARSE_FAILED_CERTIFICATE_ENCODING = -105;
    /**
     * Installation return code<br/> if the parser encountered a bad or missing package name in the
     * manifest.
     */
    public static final int INSTALL_PARSE_FAILED_BAD_PACKAGE_NAME = -106;
    /**
     * Installation return code<br/> if the parser encountered a bad shared user id name in the
     * manifest.
     */
    public static final int INSTALL_PARSE_FAILED_BAD_SHARED_USER_ID = -107;
    /**
     * Installation return code<br/> if the parser encountered some structural problem in the
     * manifest.
     */
    public static final int INSTALL_PARSE_FAILED_MANIFEST_MALFORMED = -108;
    /**
     * Installation return code<br/> if the parser did not find any actionable tags (instrumentation
     * or application) in the manifest.
     */
    public static final int INSTALL_PARSE_FAILED_MANIFEST_EMPTY = -109;
    /**
     * Installation return code<br/> if the system failed to install the package because of system
     * issues.
     */
    ...
    /**
     * Uninstall return code<br/> uninstall fail if the system failed to delete the package for an
     * unspecified reason.
     */
    public static final int DELETE_FAILED_INTERNAL_ERROR = -1;
    /**
     * Uninstall return code<br/> uninstall fail if the system failed to delete the package because
     * it is the active DevicePolicy manager.
     */
     ...
     * #installSilent(Context, String)}</li> <li>else see {@link #installNormal(Context,
     * String)}</li> </ul>
     */
    ...
    /**
     * install package silent by root <ul> <strong>Attentions:</strong> <li>Don't call this on the
     * ui thread, it may costs some times.</li> <li>You should add <strong>android.permission.INSTALL_PACKAGES</strong>
     * in manifest, so no need to request root permission, if you are system app.</li> <li>Default
     * pm install params is "-r".</li> </ul>
     *
     * @param filePath file path of package
     ...
     * install package silent by root <ul> <strong>Attentions:</strong> <li>Don't call this on the
     * ui thread, it may costs some times.</li> <li>You should add <strong>android.permission.INSTALL_PACKAGES</strong>
     * in manifest, so no need to request root permission, if you are system app.</li> </ul>
     *
     * @param filePath file path of package
     ...
     * #uninstallSilent(Context, String)}</li> <li>else see {@link #uninstallNormal(Context,
     * String)}</li> </ul>
     *
    ...
    public static boolean uninstallNormal(Context context, String packageName) {
        ...
        Intent
            i =
            new Intent(Intent.ACTION_DELETE, Uri.parse(new StringBuilder(32).append("package:")
    ...
    /**
     * uninstall package silent by root <ul> <strong>Attentions:</strong> <li>Don't call this on the
     * ui thread, it may costs some times.</li> <li>You should add <strong>android.permission.DELETE_PACKAGES</strong>
     * in manifest, so no need to request root permission, if you are system app.</li> </ul>
     *
     ...
     * @return <ul> <li>{@link #DELETE_SUCCEEDED} means uninstall success</li> <li>{@link
     * #DELETE_FAILED_INTERNAL_ERROR} means internal error</li> <li>{@link
     * #DELETE_FAILED_INVALID_PACKAGE} means package name error</li> <li>{@link
     * #DELETE_FAILED_PERMISSION_DENIED} means permission denied</li>
     */
     ...
     * whether packageName is system application
     *
     * @return <ul> <li>if packageManager is null, return false</li> <li>if package name is null or
     * is empty, return false</li> <li>if package name not exit, return false</li> <li>if package
     * name exit, but not system app, return false</li> <li>else return true</li> </ul>
     */
    ...
    /**
     * whether the app whost package's name is packageName is on the top of the stack <ul>
     * <strong>Attentions:</strong> <li>You should add <strong>android.permission.GET_TASKS</strong>
     * in manifest</li> </ul>
     *
    ...
    /**
     * get system install location<br/> can be set by System Menu Setting->Storage->Prefered install
     * location
     *
