...
/**
 * File name rule, used when saving images in {@link ImageSDCardCache} <ul> <li>use image url as
 * file name, replace char with _ if not letter or number</li> <li>use file suffix in url as target
 * file suffix</li> <li>use {@link #setFileExtension(String)} set file extension</li> </ul>
 *
...
public class FileNameRuleImageUrl implements FileNameRule {
    ...
    public String getFileName(String imageUrl) {
        ...
        String
            fileName =
            (imageUrl.length() > MAX_FILE_NAME_LENGTH ? imageUrl.substring(imageUrl.length()
