...
/**
 * 介绍RXAndroid使用的demo,这些内容很多与JavaTest中的雷同，权当再次熟悉API <p/> <p/> Observable 即被观察者，它决定什么时候触发事件以及触发怎样的事件。RxJava 使用 create() 方法来创建一个 Observable ，并为它定义事件触发规则。 <p/> Observable 并不是在创建的时候就立即开始发送事件，而是在它被订阅的时候，即当 subscribe() 方法执行的时候。 <p/> Created by yantinggeng on 2015/11/4. <p/> 内容和大部分的注释来自于：
