...
public class PullScrollView extends ScrollView {
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        // 接管自身的滑动事件的处理，当自定义逻辑认为没在滑动，就将事件交给默认的处理，起到禁止控件本身的滑动.
    ...
    private void doActionMove(MotionEvent event) {
        ...
        if (isMoving) {
            ...
            if (mContentRect.isEmpty()) {
                // 保存正常的布局位置
                mContentRect
                    .set(mContentView.getLeft(), mContentView.getTop(), mContentView.getRight(),
            ...
            // 修正content移动的距离，避免超过header的底边缘，最大的滑动距离为当前底部减去头部的可视高度
    ...
    private void rollBackAnimation() {
        ...
        mContentView
            .layout(mContentRect.left, mContentRect.top, mContentRect.right, mContentRect.bottom);
