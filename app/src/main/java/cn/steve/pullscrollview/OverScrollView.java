...
public class OverScrollView extends FrameLayout implements View.OnTouchListener {
     ...
     * focus changes initiated outside this ScrollView knows that it does not have to do anything.
     */
    ...
    public boolean executeKeyEvent(KeyEvent event) {
        ...
        if (!canScroll()) {
            if (isFocused() && event.getKeyCode() != KeyEvent.KEYCODE_BACK) {
                ...
                return nextFocused != null && nextFocused != this && nextFocused
    ...
    public boolean inChild(int x, int y) {
        if (getChildCount() > 0) {
            ...
            final View child = getChildAt(0);
            return !(y < child.getTop() - scrollY || y >= child.getBottom() - scrollY || x < child
                .getLeft() || x >= child.getRight());
        }
        return false;
    }
    ...
    public boolean isOverScrolled() {
        return (getScrollY() < child.getPaddingTop() || getScrollY() > child.getBottom() - child
    ...
    /**
     * <p> Finds the next focusable component that fits in this View's bounds (excluding fading
     * edges) pretending that this View's top is located at the parameter top. </p>
     *
    ...
    /**
     * <p> Handles scrolling in response to a "page up/down" shortcut press. This method will scroll
     * the view by one page up or down and give the focus to the topmost/bottommost component in the
     ...
     *
     * This is more expensive than the default {@link android.view.ViewGroup} implementation,
    ...
    protected boolean onRequestFocusInDescendants(int direction, Rect previouslyFocusedRect) {
        ...
        final View
            nextFocus =
            previouslyFocusedRect == null ? FocusFinder.getInstance()
    ...
    protected void onScrollChanged(int leftOfVisibleView, int topOfVisibleView,
        ...
        if (isInFlingMode) {
            ...
            if (topOfVisibleView < paddingTop || topOfVisibleView > contentBottom - displayHeight) {
                if (topOfVisibleView < paddingTop) {
                    mScroller
                        .startScroll(0, topOfVisibleView, 0, paddingTop - topOfVisibleView, 1000);
                } else if (topOfVisibleView > contentBottom - displayHeight) {
                    mScroller.startScroll(0, topOfVisibleView, 0,
                                          contentBottom - displayHeight - topOfVisibleView, 1000);
                }
        ...
        super.onScrollChanged(leftOfVisibleView, topOfVisibleView, oldLeftOfVisibleView,
                              oldTopOfVisibleView);
    }
    ...
    public void fling(int velocityY) {
        if (getChildCount() > 0) {
            ...
            if (newFocused != findFocus() && newFocused
                .requestFocus(movingDown ? View.FOCUS_DOWN : View.FOCUS_UP)) {
                mScrollViewMovedFocus = true;
                mScrollViewMovedFocus = false;
            }
    ...
    private boolean overScrollView() {
        ...
        if (currScrollY < contentTop) {
            ...
            // Scroll to content top
            if (child.getHeight() - child.getPaddingTop() - child.getPaddingBottom()
                < displayHeight) {
