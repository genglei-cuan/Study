...
public class ListViewMainActivity extends Activity {
    ...
    /**
     * 异步加载图片基本思想： 1. 先从内存缓存中获取图片显示（内存缓冲） 2. 获取不到的话从SD卡里获取（SD卡缓冲） 3. 都获取不到的话从网络下载图片并保存到SD卡同时加入内存并显示
     * 4、 采用线程池 5、 内存缓存+文件缓存 6、 内存缓存中网上很多是采用SoftReference来防止堆溢出，这儿严格限制只能使用最大JVM内存的1/4 7、
